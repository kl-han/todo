...
final List<SchemaFixture> releasedSchemas = [
  ...
  (
    version: 6,
    // Schema v6 exactly as first released in v1.7.0 (weekly snapshots).
    ddl: '''
      CREATE TABLE recurrence_rules (
        id         TEXT PRIMARY KEY,
        dtstart    TEXT NOT NULL,
    ...
    seed: (db) {
      db.execute(
        'INSERT INTO weekly_report_snapshots (week_start, generated_at, '
        "report_version, summary_json, user_notes) VALUES ('2026-07-13', "
        "'2026-07-19T20:00:00.000000Z', 1, '{}', 'solid week')",
      );
    },
    verify: (db) {
      expect(
        db
            .select('SELECT week_start, user_notes '
                'FROM weekly_report_snapshots')
            .first
            .values,
        ['2026-07-13', 'solid week'],
      );
    },
  ),
];
